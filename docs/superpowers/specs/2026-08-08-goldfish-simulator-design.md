...
- **D11 — Reports are server-rendered, never model-authored.** Every batch run
  yields a self-contained HTML report generated by the server from the actual
  run data, identified by a content-addressed run code. Claude delivers it
  verbatim (artifact or hosted URL) and must not hand-author report numbers —
  the report is the proof of what ran, extending the anti-fabrication design
  from the metrics to the presentation layer. Rejected: a template the model
  fills with data (reintroduces the fabrication surface the report exists to
  close).
...
goldfish/
  ...
  report.py      # run/AB results → self-contained HTML report (inline SVG)
...
## Run reports (proof of work)

Users will ask for proof of what ran. Per D11, `goldfish_run` and `goldfish_ab`
render a **self-contained HTML report** server-side: run fingerprint header
(deck name/commander, n, seed, engine version, date, run code), headline stat
tiles with CIs, charts as server-generated inline SVG (cast-turn histogram,
%-reached-by-turn curves, on-curve bars, casts-per-turn, paired-delta whiskers
for A/B), the trigger-fire table, and the full honesty report with the A/B
scope banner. No JavaScript, no external requests — artifact-CSP-proof by
construction; light/dark via CSS media query. Chart design follows the
project's dataviz guidance at implementation time.

- **Run code:** short base32 of `hash(deck, annotations, combos, seed, n,
  until_turn, engine_version)`. Content-addressed: identical inputs → the same
  code and (per the determinism criterion) byte-identical results, so the code
  doubles as a reproducibility claim anyone can verify by re-running. The
  report states this ("re-run with these inputs reproduces this report").
- **Delivery path 1 — artifact:** run/AB responses stay lean (text + metrics
  JSON + `run_id`); `goldfish_report(run_id)` returns the HTML, which Claude
  publishes **verbatim** as an artifact or file. Reports are cached in-memory
  (LRU, same lifecycle discipline as games).
- **Delivery path 2 — hosted (config-gated):** when `GOLDFISH_PUBLIC_BASE_URL`
  and `GOLDFISH_REPORT_DIR` are set, reports are also persisted to disk
  (content-addressed filenames, size-capped cleanup) and every run response
  includes `report_url` (e.g. `https://kautiontape.com/goldfish/r/<code>`),
  served by a GET route added to the same Starlette app the MCP transport
  runs on. Unknown codes 404. Reports are public-with-code — the same
  capability-token model as game IDs. When unconfigured, the tools simply
  omit `report_url`.
...
| `goldfish_run` | deck, annotations?, combos?, n=1000, seed=42, until_turn=10, opponents=1, mulligan opts | text summary + metrics JSON (with CIs) + honesty report + `run_id` (+ `report_url` when hosted mode is configured) |
| `goldfish_ab` | deck_a, deck_b, shared/per-deck annotations, combos?, n, seed | scope banner, paired-seed per-metric deltas ± CI, significance flags, achieved pairing correlation, honesty report + `run_id` (+ `report_url`) |
| `goldfish_report` | run_id | the server-rendered self-contained HTML report, for verbatim publication as an artifact/file |
...
- [ ] A game evicted from the in-memory store is fully restorable from its
      echoed state blob.
- [ ] Reports: the HTML makes no external requests (renders under artifact
      CSP); identical run inputs produce the same run code; every number in
      the report matches the run's metrics JSON (generated from the same
      object, never re-derived); the hosted route 404s unknown codes and is
      absent when unconfigured.

## Phasing

- **v1:** everything above (hosted report delivery is config-gated — it ships
  dark until the kautiontape.com route/env is wired, without blocking anything).
