...
- Blank-looking separator lines in the body are a single U+00A0 (non-breaking space), not empty — ~4,089 of them; only ~12 truly empty lines exist, at top-level section boundaries. Blank-line checks must use str.strip() (which strips U+00A0), never == "". Three separator lines in the fixture are U+00A0 to exercise this.
...
Note the deliberate coverage: subrule-letter skip (`704.5k` → `704.5m`), an `Example:` continuation line under `702.1`, a two-paragraph glossary entry (`Dies`), a glossary ref to a rule missing from the slice (`701.7` under `Destroy`), and `Glossary`/`Credits` appearing twice (TOC + body). Additionally, three separator lines (before 702.2c, before the Dies term, and between the two Dies paragraphs) must be a single U+00A0 character, not empty — matching the real file's separators.
