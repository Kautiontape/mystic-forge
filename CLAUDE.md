...
# Run the test suite (860 tests; -m "not slow" skips the long acceptance runs)
...
- **WATCHLIST** — passphrase-named price watchlists (`watchlist_*`, `price_history`), backed by **two** SQLite files and served as HTML pages under `/w/` and `/s/` (`/w/new` and `/w/` are the "forge" page that mints a list by hand; they are declared before `/w/{passphrase}` because Starlette matches in order). Every entry has a **price basis**: its pinned `shop`, else the cheapest across `USD_SHOPS` (Cardmarket is EUR and never mixed in). `basis_summary` + `effective_target` in `db.py` are the one place hit state is judged — the board, the tools, the CSV, and `notify_hits` all read through them, so a lit card and a push alert can never disagree. A target is fixed (`target_price`) or a rule (`target_mode='low'`: the historic low before today, less `target_pct`); the shared grammar (`2.50`, `low`, `low-10%`, `-20%`) lives in `mystic_forge/watchlist/targets.py`. Import (page and `watchlist_bulk_add`) share `_bulk_add_cards` in `server.py`; printings named on decklist lines are pinned only on request (deck exports name the copy you own). Export is browser-side: TCGplayer Mass Entry and Card Kingdom's builder take the list in a URL, Mana Pool is paste-only (`/add-deck`) so its button copies then opens. The MTGStocks hop-out badge needs a per-printing id that no dataset we ingest carries, so `mtgstocks.py` caches one per card; pages read that cache only and drop the badge when it's empty. **MTGStocks' WAF blocks DigitalOcean's whole ASN**, so on the deployed host the ingest-time resolver always fails (it still works when self-hosted from a residential connection, and logs a warning when refused). Viewers' browsers fill the cache instead: the card modal resolves the id and POSTs it to `/api/mtgstocks`, which is the one write a read-only `/s/` page may make. Nothing about a submitter is trusted — `looks_like()` requires the slug to name the card it claims, which caps a hostile submission at another printing of the same card, and votes settle that residue (most votes wins, ties keep the incumbent, one uncontested vote is enough). `source='trusted'` is never overwritten by votes
